...
class TestLink {
	...
	unassignTestCaseExecutionTask(options) {
		checkMandatoryFields(options, ["testplanid", "testcaseexternalid", "user"]);
	...
	_performRequest(method, options = {}) {
		if (options.hasOwnProperty('devKey'))
			options.devKey = options.devKey;
		...
		return new Promise((resolve, reject) => {
			this.rpcClient.methodCall('tl.' + method, [options], _handleResponse);
		});
	}
...
function checkMandatoryFields(options, mandatoryFields) {
	if (Array.isArray(options)) {
		...
		for (let field of mandatoryFields) {
			if (Array.isArray(field) && !_checkMandatoryFieldsArray(options, field))
				throw Error("Expected one of this fields: [" + field + "].")
			else if (typeof field === 'object')
				_checkMandatoryFieldsArray(options, field);
