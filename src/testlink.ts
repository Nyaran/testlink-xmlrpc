...
import {MandatoryFields} from './decorators.js';
...
export default class TestLink {
	 ...
	 * result will be got WITHOUT checking for a particular platform and build.
	 *
	 * @param options Options
	 * @param options.testplanid Test Plan id.
	...
	getLastExecutionResult(options: ApiTypes.RequestOptions & {
		...
		return this._performRequest<ApiTypes.ExecutionResult[]>('getLastExecutionResult', options);
	}
	...
	async setTestCaseExecutionType(options: ApiTypes.RequestOptions & {
		testcaseexternalid: string
		version: string | number
		testprojectid: string | number
		executiontype: ExecutionType
	}): Promise<ApiTypes.TestCase> {
		const response = await this._performRequest<ApiTypes.TestCase[]>('setTestCaseExecutionType', options);
	 ...
	 */
	_performRequest<T>(method: string, {
		apiKey,
		devKey = apiKey || this.apiKey,
		...options
	}: ApiTypes.RequestOptions = {}): Promise<T> {
		return new Promise((resolve, reject) => {
			if (this.rpcClient) {
				// eslint-disable-next-line promise/prefer-await-to-callbacks
				this.rpcClient.methodCall(`tl.${method}`, [{devKey, ...options}], (error: string, value: T | ErrorValue | ErrorValue[]) => {
					if (error) {
						reject(new Error(error));
						return;
					} else if (Array.isArray(value)) {
						for (const v of value) {
							if (Object.hasOwn(v, 'code')) {
								reject(new Error(`[${v.code}] ${v.message}`));
								return;
							}
						}
					} else if ((value as ErrorValue).status_ok === 0) {
						reject(new Error((value as ErrorValue).message));
						return;
					}
					resolve(value as T);
				});
			} else {
				reject(new Error('RPC Client not initiated.'));
			}
		});
	}
}
